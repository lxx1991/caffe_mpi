...
template <typename Dtype>
class ShowImgLayer : public Layer<Dtype> {
 public:
  explicit ShowImgLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {}

  virtual inline const char* type() const { return "ShowImg"; }
  virtual inline int MinBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 0; }
...
template <typename Dtype>
class FeatureAggregationLayer: public Layer<Dtype> {
public:
    explicit FeatureAggregationLayer(const LayerParameter& param)
            : Layer<Dtype>(param) {}
    virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                            const vector<Blob<Dtype>*>& top);
    virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
    ...
    virtual inline const char* type() const { return "FeatureAggregation"; }
    // Scale
    virtual inline int MinBottomBlobs() const { return 2; }
...
protected:
    ...
    int feature_num_, scale_dim_, inner_dim_;
};
