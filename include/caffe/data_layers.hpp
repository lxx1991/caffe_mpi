...
template <typename Dtype>
class SibDataLayer :  public Layer<Dtype>, public InternalThread {
public:
	explicit SibDataLayer(const LayerParameter& param)
	: Layer<Dtype>(param),
	  transform_param_(param.transform_param()),
	  sib_data_param_(param.sib_data_param()) {}
	virtual ~SibDataLayer();
	// LayerSetUp: implements common data layer setup functionality, and calls
	// DataLayerSetUp to do special data layer setup for individual layer types.
	// This method may not be overridden.
	void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	virtual inline const char* type() const { return "StableData"; }
...
protected:
	...
	TransformationParameter transform_param_;
	shared_ptr<LJDataTransformer<Dtype> > data_transformer_;
	shared_ptr<Caffe::RNG> rng_;
	bool has_label_;
	bool use_short_edge_;
	int short_edge_range_;
	SibDataParameter sib_data_param_;
	int sib_num_;
