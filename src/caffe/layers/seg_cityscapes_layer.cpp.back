...
void SegCityscapesLayer<Dtype>:: DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	...
	top[0]->Reshape(batch_size_, datum_data.channels() * 2, crop_height, crop_width);
	this->prefetch_data_.Reshape(batch_size_, datum_data.channels() * 2, crop_height, crop_width);
...
void SegCityscapesLayer<Dtype>::InternalThreadEntry(){
	...
	Datum datum_data, datum_data2, datum_data3, datum_label;
	...
	for (int batch_iter = 0; batch_iter < batch_size_; batch_iter++)
	{
        ...
        data_index = 0;
        for (int c = 0; c < datum_data2.channels(); ++c) {
          for (int h = 0; h < datum_data3.height(); ++h) {
            for (int w = 0; w < datum_data3.width(); ++w) {
              datum_data_string->push_back(
                static_cast<uint8_t>(datum_data2.data()[data_index++]));
            }
          }
        }
		...
		if (false)
		{
		  	...
		  	for (int p1 = 0; p1 < this->prefetch_data_.height(); p1 ++)
		  		for (int p2 = 0; p2 < this->prefetch_data_.width(); p2 ++)
		  		{
		  			im_data2.at<uchar>(p1, p2*3+0) = (uchar)(this->prefetch_data_.data_at(0, 3, p1, p2)+104);
		  			im_data2.at<uchar>(p1, p2*3+1) = (uchar)(this->prefetch_data_.data_at(0, 4, p1, p2)+117);
		  			im_data2.at<uchar>(p1, p2*3+2) = (uchar)(this->prefetch_data_.data_at(0, 5, p1, p2)+123);	
		  		}
	  		for (int p1 = 0; p1 < this->prefetch_label_.height(); p1 ++)
	  	  		for (int p2 = 0; p2 < this->prefetch_label_.width(); p2 ++)
