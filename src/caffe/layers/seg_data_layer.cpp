...
void SegDataLayer<Dtype>:: DataLayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
	...
	int crop_height = ((datum_data.height() - 1) / stride + 1) * stride;
	int crop_width = ((datum_data.width() - 1) / stride + 1) * stride;
			...
			for (int t = 0; t < 20; t++)
			{
				...
				for (int p1 = 0; p1 < this->prefetch_label_.height(); p1 ++)
		  	  		for (int p2 = 0; p2 < this->prefetch_label_.width(); p2 ++)
		  	  			for (int p3 = 0; p3 < this->prefetch_label_.channels(); p3 ++)
			  	  		{
			  	  			int label_value = (int)this->prefetch_label_.data_at(batch_iter, p3, p1, p2);
			  	  			cnt[label_value]++;
			  	  		}
		  	  	for (int i = 0; i<cnt.size(); i++)
		  			max_label_cnt = std::max(max_label_cnt, cnt[i]);

		  		if (max_label_cnt > 0.8 * this->prefetch_label_.count(1))
		  			this->data_transformer_->Transform(datum_data, datum_label, &this->prefetch_data_, &this->prefetch_label_, batch_iter);
	  			else
	  				break;
			}
