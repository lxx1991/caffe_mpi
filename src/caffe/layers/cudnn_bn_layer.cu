...
void CuDNNBNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->phase_ == TEST || this->frozen_) {
    ...
    if (this->rebn_)
    {
      ...
      // Multiply r
      caffe_gpu_mul(this->broadcast_buffer_.count(), top[0]->gpu_data(),
          this->broadcast_buffer_.gpu_data(), top_data);
     ...
     caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, this->num_, this->channels_, 1,
      ...
      caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, this->num_ * this->channels_,
          ...
          Dtype(0), this->broadcast_buffer_.mutable_gpu_data());
      // Add d
      caffe_gpu_add(this->broadcast_buffer_.count(), top[0]->gpu_data(),
          this->broadcast_buffer_.gpu_data(), top_data); 
