...
void LSRLossLayer<Dtype>::LayerSetUp(
  ...
  thresh_ratio_ = this->layer_param_.loss_param().thresh_ratio();
}

template <typename Dtype>
void LSRLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LSRLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void LSRLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        const int label_value = static_cast<int>(label[i * inner_num_ + j]);
        const Dtype loss = loss_vec[i * inner_num_ + j];
        if (loss < threshold || (has_ignore_label_ && label_value == ignore_label_)) {
          ...
          for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
            bottom_diff[i * dim + c * inner_num_ + j] -= Dtype(0.000274);
          }
          bottom_diff[i * dim + label_value * inner_num_ + j] -= 0.9;
          ++count;
        }
      }
    }
...
#ifdef CPU_ONLY
STUB_GPU(LSRLossLayer);
#endif

INSTANTIATE_CLASS(LSRLossLayer);
REGISTER_LAYER_CLASS(LSRLoss);
