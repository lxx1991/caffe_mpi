...
void MultiLabelLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // Commented on 2nd, August, 2014 by Ziwei
      loss -= input_data[i] * ((target[i] > 0) - (input_data[i] >= 0)) -
         log(1 + exp(input_data[i] - 2 * input_data[i] * (input_data[i] >= 0)));

    // Modified on 2nd, August, 2014 by Ziwei
    //  int weight_pos = 3;

    //  if (target[i] > 0){
    //    loss -= weight_pos * input_data[i] * ((target[i] > 0) - (input_data[i] >= 0)) -
    //      log(1 + exp(input_data[i] - 2 * input_data[i] * (input_data[i] >= 0)));
    //  }
    //  else {
    // 	  loss -= input_data[i] * ((target[i] > 0) - (input_data[i] >= 0)) -
...
void MultiLabelLossLayer<Dtype>::Backward_cpu(
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < count; ++i) {
      ...
      //  if (target[i] > 0) {
      //    bottom_diff[i] = weight_pos * (sigmoid_output_data[i] - (target[i] > 0));
      //  }
      //  else {
      //    bottom_diff[i] = sigmoid_output_data[i] - (target[i] > 0);
      //  }
