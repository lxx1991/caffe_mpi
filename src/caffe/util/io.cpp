...
bool ReadSegVideoToDatum(const vector<string>& img_filename, const vector<string>& label_filename, Datum* datum_data, Datum* datum_label, bool is_color) {
  ...
  {
    cv::Mat cv_label = cv::imread(label_filename[i], CV_LOAD_IMAGE_GRAYSCALE);

    if (!cv_label.data){
      LOG(ERROR) << "Could not load file " << label_filename[i];
      return false;
    }
