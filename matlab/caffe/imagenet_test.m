function imagenet_test(path)
%path = '.';
path = '/home/sqiu/dataset/ILSVRC2012/val_resize_2';
...
% get ground truth
gt_file = '/home/sqiu/dataset/ILSVRC2012/val.txt';
fid = fopen(gt_file);
gt_label = zeros(50000,1);
for id = 1 : 50000
    filename = fscanf(fid, '%s', 1);
    gt_label(id) = fscanf(fid, '%d', 1);
end
fclose(fid);

% get the predicting labels
batch_num = ceil(length(filelist) / 64);
predicting = zeros(64 * (batch_num - 1), 1);
gt_label =  gt_label(1 : 64 * (batch_num - 1));
...
for b = 1 : batch_num -1 
   ...
   [~ , predicting((b-1)*64+1:(b-1)*64+bsize)] = max(batch_feature); 
end

predicting = predicting - 1;
acc = sum(gt_label == predicting) / length(gt_label);
fprintf('acc is : %f\n', acc);
C = confusionMatrix(gt_label, predicting);
save('C', 'C');
% ------------------------------------------------------------------------
